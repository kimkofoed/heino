      ...
      try {
        ...
        switch (data.event) {
          case 'start':
            ...
            // 🔥 Keep-Alive stilhed (Twilio lukker ellers forbindelsen efter få sekunder)
            session.keepAlive = setInterval(() => {
              const silence = {
                event: 'media',
                streamSid: session.streamSid,
                media: { payload: Buffer.alloc(320).toString('base64') }, // ~20ms stilhed
              };
              connection.send(JSON.stringify(silence));
            }, 1000);

            // 🔥 Når streamen er klar, lad OpenAI tale
            const greeting = {
...
async function processTranscriptAndSend(transcript, sessionId = null) {
  try {
    ...
    console.log('Extracted details:', parsed);
